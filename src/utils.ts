...
import { Currency, PlannedExpense, Debt, ScheduledPayment, SavingsGoal } from './types';
...
export function getSavingsGoalsWithDeadlineInMonth(goals: SavingsGoal[], year: number, month: number): { [dateStr: string]: SavingsGoal[] } {
  const result: { [dateStr: string]: SavingsGoal[] } = {};
  for (const goal of goals) {
    if (goal.status === 'active') {
      const d = parseISO(goal.deadline);
      if (d.getFullYear() === year && d.getMonth() === month) {
        if (!result[goal.deadline]) result[goal.deadline] = [];
        result[goal.deadline].push(goal);
      }
    }
  }
  return result;
...
export function getSavingsGoalsForDate(goals: SavingsGoal[], dateStr: string): SavingsGoal[] {
  return goals.filter((g) => g.deadline === dateStr && g.status === 'active');
}
