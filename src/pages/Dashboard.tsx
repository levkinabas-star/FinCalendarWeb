...
import ContributeToSavingsGoal from '../components/ContributeToSavingsGoal';
import { Transaction, Debt, DebtPayment, PlannedExpense, ScheduledPayment, Category, Account, Currency, SavingsGoal } from '../types';
import { useNavigate } from 'react-router-dom';

export default function Dashboard() {
  const {
    language, accounts, transactions, categories, plannedExpenses, defaultCurrency,
    budgets, debts, savingsGoals,
    deleteTransaction, addTransaction, deletePlannedExpense, deleteDebtPayment, deleteSavingsGoal,
    revertDebtPaymentToScheduled, togglePlannedCompleted, markPlannedCompletedNoDeduction,
    markScheduledCompletedNoDeduction, unmarkScheduledCompleted, markScheduledAsPaid, markSavingsGoalCompleted,
    ...
    deletePlannedExpense: s.deletePlannedExpense,
    deleteDebtPayment: s.deleteDebtPayment,
    deleteSavingsGoal: s.deleteSavingsGoal,
    revertDebtPaymentToScheduled: s.revertDebtPaymentToScheduled,
    togglePlannedCompleted: s.togglePlannedCompleted,
    markPlannedCompletedNoDeduction: s.markPlannedCompletedNoDeduction,
    markScheduledCompletedNoDeduction: s.markScheduledCompletedNoDeduction,
    unmarkScheduledCompleted: s.unmarkScheduledCompleted,
    markScheduledAsPaid: s.markScheduledAsPaid,
    markSavingsGoalCompleted: s.markSavingsGoalCompleted,
  })));
  ...
  const [deletingSavingsGoal, setDeletingSavingsGoal] = useState<SavingsGoal | null>(null);
    ...
    <div className="page-enter pb-32">
      ...
      <div className="mx-5 mb-5 p-5 rounded-3xl relative overflow-hidden"
        ...
        <p className="text-4xl font-bold text-white mb-4 relative z-10">
          {formatAmount(accounts.reduce((s, a) => s + a.balance, 0), defaultCurrency as Currency)}
        </p>
        ...
        <div className="flex gap-4 relative z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-emerald-400">+{formatAmount(totalMonthIncome, defaultCurrency as Currency)}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-red-400">-{formatAmount(totalMonthExpense, defaultCurrency as Currency)}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={`text-sm font-bold ${totalMonthIncome - totalMonthExpense >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {totalMonthIncome - totalMonthExpense >= 0 ? '+' : '-'}
                {formatAmount(Math.abs(totalMonthIncome - totalMonthExpense), defaultCurrency as Currency)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mx-5 mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          {savingsGoals.length > 0 && (
            <button
              ...
              className="text-blue-400 text-xs font-medium flex items-center gap-0.5"
            >
              {t.addSavingsGoal} <ChevronRight size={13} />
            </button>
          )}
        </div>
        ...
        {savingsGoals.length === 0 ? (
          <button
            onClick={() => setAddingSavingsGoal(true)}
            className="w-full rounded-2xl p-4 flex items-center gap-3 active-scale"
            style={{ background: '#0E0E1C', border: '1px solid #1E2A40' }}
          >
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: `${'#3B82F6'}20` }}>
              <span className="text-xl">🎯</span>
            </div>
            <div className="text-left">
              <p className="text-sm font-medium text-slate-200">
                {language === 'ru' ? 'Создать цель накоплений' : 'Create savings goal'}
              </p>
              <p className="text-xs text-slate-500">
                {language === 'ru' ? 'Накопите на важную покупку' : 'Save for something important'}
              </p>
            </div>
            <ChevronRight size={16} className="text-slate-500 ml-auto" />
          </button>
          ...
          <div className="space-y-2">
            {savingsGoals.slice(0, 3).map((goal) => {
              ...
              const daysLeft = Math.ceil((parseISO(goal.deadline).getTime() - Date.now()) / 86400000);
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1.5 h-1.5 rounded-full overflow-hidden" style={{ background: '#1E2A40' }}>
                      <div
                        ...
                        style={{ width: `${progress}%`, background: isCompleted ? '#10B981' : goal.color }}
                      />
                    </div>
                    <div className="flex items-center justify-between mt-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md" style={{ background: `${goal.color}20`, color: isCompleted ? '#10B981' : goal.color }}>
                          {Math.round(progress)}%
                        </span>
                      </div>
                      <p className={`text-[10px] font-medium ${isOverdue ? 'text-red-400' : 'text-slate-400'}`}>
                        {isOverdue
                          ? (language === 'ru' ? 'Просрочено' : 'Overdue')
                          : (language === 'ru' ? `${daysLeft} дн.` : `${daysLeft}d left`)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      onClick={() => setContributingToGoal(goal)}
                      className="px-3 py-2 rounded-xl text-xs font-semibold active-scale"
                      style={{ background: `${goal.color}25`, color: goal.color }}
                    >
        ...
        {savingsGoals.length > 3 && (
          <button
            ...
            className="w-full mt-3 py-2.5 rounded-2xl text-sm font-medium text-blue-400 active-scale"
            style={{ background: '#1E1E38', border: '1px solid #1E2A40' }}
          >
            {language === 'ru' ? `Все цели (${savingsGoals.length})` : `All goals (${savingsGoals.length})`}
          </button>
        )}
      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">
                      {formatAmount(b.spent, defaultCurrency as Currency)} / {formatAmount(b.limit, defaultCurrency as Currency)}
                    </p>
                  </div>
                </div>
              );
      ...
      <Modal
        isOpen={addingSavingsGoal}
        onClose={() => setAddingSavingsGoal(false)}
        title={t.addSavingsGoal}
        fullHeight
      >
        <SavingsGoalForm onClose={() => setAddingSavingsGoal(false)} />
      </Modal>
      ...
      {/* Contribute to Savings Goal Modal */}
      <Modal
        isOpen={!!contributingToGoal}
        onClose={() => setContributingToGoal(null)}
        title={t.savingsGoalContributeTitle}
        fullHeight
      >
        {contributingToGoal && (
          ...
          <div className="flex items-center gap-1">
            {onPayDebtScheduled && !payment.completedDates?.includes(date) && (
              <button
                ...
                className="w-9 h-9 rounded-lg flex items-center justify-center active-scale"
