...
import {
	formatAmount,
	getDatesWithEventsInMonth,
	getExpensesForDate,
	getDebtPaymentsInMonth,
	getDebtPaymentsForDate,
	getScheduledPaymentsInMonth,
	getScheduledPaymentsForDate,
	getDebtsWithDueDateInMonth,
	type DebtScheduledPayment,
	getSavingsGoalsWithDeadlineInMonth,
	getSavingsGoalsForDate,
} from "../utils";
import type {
	PlannedExpense,
	Transaction,
...
export default function Calendar() {
	...
	const [deletingScheduledPayment, setDeletingScheduledPayment] = useState<{
		debtId: string;
		scheduledId: string;
		personName: string;
	} | null>(null);
	...
	const [deletingSavingsGoal, setDeletingSavingsGoal] =
		useState<SavingsGoal | null>(null);
	...
	const selectedScheduledPayments = getScheduledPaymentsForDate(
		debts,
		selectedDate,
	);
	const debtsWithDueDateMap = getDebtsWithDueDateInMonth(debts, year, month);
	const savingsGoalsMap = getSavingsGoalsWithDeadlineInMonth(
		savingsGoals,
		year,
		month,
	);
	const selectedSavingsGoals = getSavingsGoalsForDate(
		savingsGoals,
		selectedDate,
	);

	// Map of dates with completed (paid) scheduled payments — for green dots in calendar
	const paidScheduledDates = useMemo(() => {
		const set = new Set<string>();
		debts.forEach((debt) => {
			(debt.scheduledPayments ?? []).forEach((sp) => {
				if (sp.paidDate) set.add(sp.dueDate);
			});
		});
		return set;
	}, [debts]);
	...
	// Unified operations: planned expenses + scheduled (unpaid only) + standalone debt payments (not from scheduled)
	type DayOp =
	...
	const dayOps = useMemo<DayOp[]>(() => {
		const planned: DayOp[] = selectedEvents.map((e) => ({
			kind: "planned",
			expense: e,
		}));
		// Include ALL scheduled payments (both unpaid and paid) — paid ones show as completed
		const scheduled: DayOp[] = selectedScheduledPayments.map((sp) => ({
			kind: "scheduled",
			sp,
			debt: sp.debt,
		}));
		// Debt payments that are NOT linked to a paid scheduled payment (to avoid duplicates)
		const debtPayments: DayOp[] = selectedDebtPayments
			.filter((p) => !p.scheduledPaymentId)
			.map((p) => ({ kind: "debtPayment", payment: p, debt: p.debt }));
		return [...planned, ...scheduled, ...debtPayments];
	...
	const hasAnything =
		hasDayOps ||
		paidScheduledDates.has(selectedDate) ||
		selectedTx.length > 0 ||
		(debtsWithDueDateMap[selectedDate]?.length ?? 0) > 0 ||
	...
	return (
		<div className="page-enter pb-32">
			...
			<div className="flex items-center justify-between px-5 pt-6 pb-4">
				...
				<button
					onClick={() =>
						canAddPlannedExpense ? setShowAdd(true) : navigate("/pricing")
					}
					...
					{days.map((day, idx) => {
						...
						return (
							<button
								...
								style={{
									borderRight:
										(idx + 1) % 7 !== 0 ? "1px solid #1E2A40" : "none",
									borderBottom:
										idx < days.length - 7 ? "1px solid #1E2A40" : "none",
									background: selected
								...
								{inMonth &&
									(events ||
										...
										<div className="flex gap-0.5 mt-0.5">
											...
											{debtPaymentInfo && !paidScheduledDates.has(dateStr) && (
												<div
													className="w-1.5 h-1.5 rounded-full"
											...
											{scheduledPaymentsMap[dateStr] &&
												!paidScheduledDates.has(dateStr) && (
													<div
														className="w-1.5 h-1.5 rounded-full"
			...
			<div className="flex items-center gap-4 px-5 mb-5 flex-wrap">
				...
				<div className="flex items-center gap-1.5">
					...
					/>
					<span className="text-xs text-slate-500">
						{language === "ru" ? "Срок долга" : "Due date"}
					</span>
				</div>
				<div className="flex items-center gap-1.5">
					<div
			...
			<div className="mx-5 space-y-4">
				...
				{!hasAnything && (
					<div
						className="rounded-2xl flex flex-col items-center py-8 text-slate-500"
						style={{ background: "#0E0E1C", border: "1px solid #1E2A40" }}
					>
				...
				{dayOps.length > 0 && (
					<div>
						...
						<div
							...
							style={{ border: "1px solid #1E2A40" }}
						>
							{dayOps.map((op, idx) => {
								const isPending =
									op.kind === "scheduled" &&
									!op.sp.paidDate &&
									!op.sp.completedDates?.includes(selectedDate);
								const isDone =
									op.kind === "planned"
										? op.expense.completedDates.includes(selectedDate)
										: op.kind === "debtPayment" ||
											(op.kind === "scheduled" && !!op.sp.paidDate);
								...
								if (op.kind === "planned") {
									...
									const isLent = op.debt.direction === "lent";
									const acc = op.sp.sourceAccountId
										? accounts.find((a) => a.id === op.sp.sourceAccountId)
										: null;
									icon = isDone ? (
								...
								return (
									<div
										...
										style={{
											background: rowBg,
											borderBottom:
												idx < dayOps.length - 1 ? "1px solid #1E2A40" : "none",
										}}
									>
										<div
										...
										<div className="flex items-center gap-1.5 flex-shrink-0">
											...
											{op.kind === "planned" && !isDone && (
												<button
													onClick={() =>
														togglePlannedCompleted(op.expense.id, selectedDate)
													}
											...
											{op.kind === "scheduled" && isPending && (
												<button
													onClick={() => {
														const accId =
															op.sp.sourceAccountId || accounts[0]?.id;
														if (accId)
															markScheduledAsPaid(op.debt.id, op.sp.id, accId);
													}}
													className="px-3 py-1.5 rounded-xl text-xs font-semibold active-scale"
													style={{
														background: "rgba(59,130,246,0.15)",
											...
											{op.kind === "scheduled" && isDone && (
												<button
													onClick={() => {
														const debtPayment = op.sp.debt.payments.find(
															(p: DebtPayment) =>
																p.scheduledPaymentId === op.sp.id,
														);
														if (debtPayment)
															revertDebtPaymentToScheduled(
																op.debt.id,
											...
											{op.kind === "planned" && (
												<>
													<div className="relative">
														...
														{openEditMenu?.key === `planned-${op.expense.id}` &&
															createPortal(
																<>
																	<div
																		className="fixed inset-0"
																		style={{ zIndex: 300 }}
																		onClick={() => setOpenEditMenu(null)}
																	/>
																	<div
																		className="fixed rounded-xl overflow-hidden shadow-lg"
																		style={{
																			top: openEditMenu.pos.top,
																			right: openEditMenu.pos.right,
																			zIndex: 301,
																			background: "#1E1E38",
													...
													<button
														onClick={() =>
															setDeletingDebtPayment({
																debtId: op.debt.id,
																paymentId: op.payment.id,
																personName: op.debt.personName,
															})
														}
														className="w-7 h-7 rounded-lg flex items-center justify-center active-scale"
														style={{ background: "#1E1E38" }}
											...
											{op.kind === "scheduled" && isPending && (
												<>
													<div className="relative">
														...
														{openEditMenu?.key === `scheduled-${op.sp.id}` &&
															createPortal(
																<>
																	<div
																		className="fixed inset-0"
													...
													<button
														onClick={() =>
															setDeletingScheduledPayment({
																debtId: op.debt.id,
																scheduledId: op.sp.id,
																personName: op.debt.personName,
															})
														}
														className="w-7 h-7 rounded-lg flex items-center justify-center active-scale"
				...
				{selectedTx.length > 0 && (
					<div>
								...
								return (
									...
									>
										...
										<div className="flex items-center gap-1.5 flex-shrink-0">
											...
											<button
												onClick={() =>
													isTransfer ? setEditingTransfer(tx) : setEditingTx(tx)
												}
						...
						{/* Day totals */}
						{(() => {
							const inc = selectedTx
								.filter((tx) => tx.type === "income")
								.reduce((s, tx) => s + tx.amount, 0);
							const exp = selectedTx
				...
				{debtsWithDueDateMap[selectedDate] &&
					debtsWithDueDateMap[selectedDate].length > 0 && (
						<div>
									...
									return (
										<div
											...
											style={{
												background: "#0E0E1C",
												borderBottom:
													idx < debtsWithDueDateMap[selectedDate].length - 1
														? "1px solid #1E2A40"
														: "none",
											}}
										>
			...
			{/* Savings Goals for the selected date */}
			{selectedSavingsGoals.length > 0 && (
				<div>
					<p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 px-1 flex items-center gap-1">
					...
					<div
						className="rounded-2xl overflow-hidden"
						style={{ border: "1px solid #14B8A630" }}
					>
						{selectedSavingsGoals.map((goal, idx) => {
							const progress = Math.min(
								(goal.currentAmount / goal.targetAmount) * 100,
								100,
							);
							const daysLeft = Math.ceil(
								(parseISO(goal.deadline).getTime() - Date.now()) / 86400000,
							);
							...
							return (
								<div
									...
									style={{
										background: "#0E0E1C",
										borderBottom:
											idx < selectedSavingsGoals.length - 1
												? "1px solid #1E2A40"
												: "none",
									}}
								>
									...
									<div className="flex-1 min-w-0">
										...
										<p className="text-xs text-slate-500">
											...
											{isOverdue
												...
												: ` · ${language === "ru" ? `${daysLeft} дн.` : `${daysLeft}d`}`}
										</p>
									</div>
									<div className="flex items-center gap-2 flex-shrink-0">
										<div
										...
										>
											<div
												...
												style={{
													width: `${progress}%`,
													background: isCompleted ? "#10B981" : goal.color,
												}}
											/>
										</div>
										<button
											onClick={() => setEditingSavingsGoal(goal)}
											className="w-7 h-7 rounded-lg flex items-center justify-center active-scale"
											style={{ background: "#1E1E38" }}
										>
											<Pencil size={12} className="text-slate-400" />
										...
										<button
											onClick={() => setDeletingSavingsGoal(goal)}
											className="w-7 h-7 rounded-lg flex items-center justify-center active-scale"
											style={{ background: "#1E1E38" }}
										>
											<Trash2 size={12} color="#EF4444" />
			...
			<Modal
				isOpen={!!deletingScheduledPayment}
				onClose={() => setDeletingScheduledPayment(null)}
				title={t.areYouSure}
			>
				<div className="px-5 pb-6">
					<p className="text-slate-400 text-sm mb-5">
					...
					<div className="flex gap-3">
						<button
							onClick={() => setDeletingScheduledPayment(null)}
							className="flex-1 py-3 rounded-2xl font-medium text-slate-300 active-scale"
							style={{ background: "#1E1E38", border: "1px solid #1E2A40" }}
						>
							{t.cancel}
						...
						<button
							onClick={() => {
								if (deletingScheduledPayment) {
									deleteScheduledPayment(
										deletingScheduledPayment.debtId,
										deletingScheduledPayment.scheduledId,
									);
			...
			<Modal
				...
				title={
					language === "ru" ? "Редактировать операцию" : "Edit Transaction"
				}
				fullHeight
			>
				{editingTx && (
			...
			<Modal
				isOpen={!!editingSavingsGoal}
				onClose={() => setEditingSavingsGoal(null)}
				title={t.editSavingsGoal}
				fullHeight
			>
				{editingSavingsGoal && (
					<SavingsGoalForm
						key={editingSavingsGoal.id}
			...
			{/* Delete Savings Goal Confirm */}
			<Modal
				isOpen={!!deletingSavingsGoal}
				onClose={() => setDeletingSavingsGoal(null)}
				title={t.areYouSure}
			>
				<div className="px-5 pb-6">
					<p className="text-slate-400 text-sm mb-5">
						{language === "ru"
							? "Цель накоплений будет удалена."
							: "Savings goal will be deleted."}
					</p>
					<div className="flex gap-3">
						<button
							onClick={() => setDeletingSavingsGoal(null)}
							className="flex-1 py-3 rounded-2xl font-medium text-slate-300 active-scale"
							style={{ background: "#1E1E38", border: "1px solid #1E2A40" }}
						>
							{t.cancel}
						...
						<button
							onClick={() => {
								if (deletingSavingsGoal) {
									deleteSavingsGoal(deletingSavingsGoal.id);
									setDeletingSavingsGoal(null);
								}
							}}
							className="flex-1 py-3 rounded-2xl font-semibold text-white active-scale"
							style={{
