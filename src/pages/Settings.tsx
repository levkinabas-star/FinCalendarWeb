...
export default function Settings() {
	...
	const { canAddCustomCategory, isPro, proExpiry } = usePlan();
		...
		<div className="page-enter pb-32">
			...
			<div className="mx-5 mb-3">
				...
				>
					...
					<div className="text-left flex-1">
						<p className="text-sm font-medium text-slate-200">
							{language === "ru" ? "Сбросить балансы" : "Reset Balances"}
						</p>
						<p className="text-xs text-slate-500">
							{language === "ru"
								? "Обнулить все операции и балансы, сохранить счета, категории, долги и историю выполненных"
								: "Reset all operations and balances, keep accounts, categories, debts and completed history"}
						</p>
					</div>
					<ChevronRight size={16} className="text-slate-500" />
				</button>
			...
			{/* Reset Balances Confirm */}
			<Modal
				isOpen={showResetConfirm}
				onClose={() => setShowResetConfirm(false)}
				title={language === "ru" ? "Сбросить балансы?" : "Reset Balances?"}
			>
				<div className="px-5 pb-6">
					<p className="text-slate-400 text-sm mb-5">
						{language === "ru"
							? "Это обнулит все балансы счетов и удалит все транзакции. Запланированные расходы, бюджеты, долги (с историей платежей) и история выполненных сохранятся."
							: "This will reset all account balances to zero and delete all transactions. Planned expenses, budgets, debts (with payment history) and completed history will be kept."}
					</p>
