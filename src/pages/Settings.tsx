...
export default function Settings() {
	...
	const [downloadTab, setDownloadTab] = useState<"android" | "iphone">(
		"android",
	);
	...
	const [newCatType, setNewCatType] = useState<"income" | "expense" | "both">(
		"expense",
	);
	...
	return (
		<div className="page-enter pb-32">
				...
				{isPro ? (
						...
						>
							...
							<div className="flex-1">
								<p className="text-sm font-bold text-white">
									{language === "ru" ? "Pro активен" : "Pro Active"}
								</p>
					...
					>
						...
						<div className="text-left flex-1">
							...
							<p className="text-xs text-slate-400">
								{language === "ru"
									? "Статистика, бюджеты, долги, экспорт и многое другое"
									: "Statistics, budgets, debts, exports & more"}
							</p>
						</div>
						<ChevronRight size={16} className="text-slate-500" />
					</button>
			...
			{/* Widgets */}
			<Section
				title={language === "ru" ? "Виджеты" : "Widgets"}
			...
			<Section title={t.defaultCurrency} icon={<DollarSign size={16} />}>
				...
				>
					<span className="text-slate-200 font-medium">
						{defaultCurrency} —{" "}
						{CURRENCY_NAMES[defaultCurrency as Currency][language]}
					</span>
					<ChevronRight size={16} className="text-slate-500" />
				</button>
			</Section>
			...
			>
				...
				>
					...
					<div className="text-left flex-1">
						...
						<p className="text-xs text-slate-500">
							{language === "ru"
								? "PDF, Excel, CSV, JSON резервная копия"
								: "PDF, Excel, CSV, JSON backup"}
						</p>
					</div>
					<ChevronRight size={16} className="text-slate-500" />
				</button>
			...
			{/* Download */}
			<Section
				title={language === "ru" ? "Скачать приложение" : "Download App"}
			...
			>
				...
				>
					...
					<div className="text-left flex-1">
						<p className="text-sm font-medium text-slate-200">
							{language === "ru"
								? "Android APK / Инструкция для iOS"
								: "Android APK / iOS Guide"}
						</p>
						<p className="text-xs text-slate-500">
							{language === "ru"
								? "Установить как нативное приложение"
				...
				>
					...
					>
						...
						<div className="text-left">
							...
							<p className="text-xs text-slate-500">
								{language === "ru"
									? "Удалить все счета, транзакции и планы"
									: "Delete all accounts, transactions and plans"}
							</p>
						</div>
					</button>
				</div>
			...
			{/* About */}
			<div
				className="mx-5 mb-5 px-4 py-4 rounded-2xl text-center"
				...
				<div className="px-5 pb-8 space-y-2">
					{ALL_CURRENCIES.map((cur) => (
						<button
							...
							style={{
								background:
									defaultCurrency === cur ? "rgba(59,130,246,0.15)" : "#1E1E38",
								border:
									defaultCurrency === cur
										? "1.5px solid #3B82F6"
										: "1.5px solid #1E2A40",
						...
						>
							<span
								...
								style={{
									color: defaultCurrency === cur ? "#3B82F6" : "#94A3B8",
									minWidth: 36,
								}}
							>
								{cur}
				...
				<div className="px-5 pb-8 space-y-4">
						...
						<div className="grid grid-cols-8 gap-2">
							{ICONS.map((ic) => (
								<button
									...
									style={{
										background:
											newCatIcon === ic ? "rgba(59,130,246,0.2)" : "#1E1E38",
										border:
											newCatIcon === ic
												? "2px solid #3B82F6"
					...
					>
						<div
							...
							style={{ background: `${newCatColor}22` }}
						>
				...
				<div className="px-5 pb-8">
					...
					{downloadTab === "android" && (
						<div className="space-y-4">
							<div
								className="flex items-center gap-3 px-4 py-4 rounded-2xl"
								style={{ background: "#0F1E0F", border: "1px solid #1A3A1A" }}
							>
								<div
								...
								<div>
									...
									<p className="text-xs text-slate-400">
										{language === "ru"
											? "Размер: 3.7 МБ • APK файл"
											: "Size: 3.7 MB • APK file"}
									</p>
								</div>
							</div>
							...
							>
								...
								{[
									...
									language === "ru"
										? "2. Откройте файл — браузер или проводник попросит разрешение"
										: "2. Open the file — browser or file manager will ask for permission",
									language === "ru"
					...
					{downloadTab === "iphone" && (
						<div className="space-y-4">
							<div
								className="flex items-center gap-3 px-4 py-4 rounded-2xl"
								style={{ background: "#0F1525", border: "1px solid #1A2540" }}
							>
								<div
									className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
								...
								{[
									{
										...
										desc:
											language === "ru"
												? "Приложение должно быть открыто именно в Safari, не Chrome или Firefox"
												: "The app must be opened in Safari, not Chrome or Firefox",
									},
									{
										icon: "⬆️",
										title:
										...
										desc:
											language === "ru"
												? "Кнопка в виде квадрата со стрелкой вверх — внизу экрана"
												: "The square with an upward arrow button at the bottom of the screen",
									},
									{
										icon: "📲",
										title:
										...
										desc:
											language === "ru"
												? "Прокрутите вниз в меню и выберите этот пункт"
												: "Scroll down in the share menu and select this option",
									},
									{
										icon: "✅",
										title:
											language === "ru" ? "Нажмите «Добавить»" : "Tap «Add»",
										desc:
											language === "ru"
												? "Иконка FinCalendar появится на экране «Домой»"
												: "FinCalendar icon will appear on your Home Screen",
									},
								].map((step, i) => (
									<div
										key={i}
