...
export default function DebtPaymentForm({ debt, onClose }: Props) {
  const { language, accounts, addDebtPayment, addScheduledPayment, deleteDebt, deleteScheduledPayment, markScheduledAsPaid } = useStore(useShallow((s) => ({
    language: s.language,
    accounts: s.accounts,
    addDebtPayment: s.addDebtPayment,
    addScheduledPayment: s.addScheduledPayment,
    deleteDebt: s.deleteDebt,
    deleteScheduledPayment: s.deleteScheduledPayment,
  ...
  return (
    <div className="px-5 pb-8">
      ...
      <div className="flex rounded-2xl p-1 gap-1 mb-5" style={{ background: '#0F0F23' }}>
        {([['pay', language === 'ru' ? 'Оплата' : 'Pay'], ['schedule', language === 'ru' ? 'Запланировать' : 'Schedule'], ['history', language === 'ru' ? 'История' : 'History']] as ['pay' | 'schedule' | 'history', string][]).map(([v, label]) => (
          <button
            key={v}
            onClick={() => setTab(v)}
