...
export default function DebtForm({ debt, onClose }: Props) {
	const { language, defaultCurrency, accounts, addDebt, updateDebt } =
		useStore();
	...
	const [paymentType, setPaymentType] = useState<"single" | "installment">(
		"single",
	);
	...
	function validate() {
		...
		if (!amount || isNaN(Number(amount)) || Number(amount) <= 0)
			e.amount = t.amountRequired;
		if (!debt && paymentType === "installment") {
			if (
				!installmentAmount ||
				isNaN(Number(installmentAmount)) ||
		...
		if (debt) {
			...
			const installments =
				paymentType === "installment"
					? generateInstallments().map((p) => ({
							...p,
							sourceAccountId: p.sourceAccountId || accountId || undefined,
						}))
					: undefined;
			...
			<div className="px-5 pb-8 flex flex-col items-center gap-5 pt-8">
				...
				<div className="text-center">
					<p className="text-slate-100 font-bold text-lg mb-2">
						{isRu
							? `Лимит: ${FREE_LIMITS.debts} долга`
							: `Limit: ${FREE_LIMITS.debts} debts`}
					</p>
		...
		<div className="px-5 pb-8 space-y-5">
				...
				<div>
					...
					<div className="flex gap-2 flex-wrap">
						<button
							...
							style={{
								background:
									accountId === "" ? "rgba(100,116,139,0.2)" : "#1E1E38",
								border:
									accountId === ""
										? "1.5px solid #64748B"
										: "1.5px solid #1E2A40",
						...
						{accounts.map((acc) => (
							<button
								...
								style={{
									background:
										accountId === acc.id ? `${acc.color}22` : "#1E1E38",
									border:
										accountId === acc.id
											? `1.5px solid ${acc.color}`
											: "1.5px solid #1E2A40",
								}}
							>
								<span>{acc.icon}</span>
					...
					{accountId && (
						<p
							...
							style={{ color: direction === "lent" ? "#EF4444" : "#10B981" }}
						>
							{direction === "lent"
			...
			<div>
				...
				<input
					...
					placeholder={
						language === "ru" ? "За что, причина..." : "Reason, notes..."
					}
					className="w-full px-4 py-3 rounded-2xl text-slate-100 placeholder-slate-600 outline-none"
					style={{ background: "#1E1E38", border: "1px solid #1E2A40" }}
				/>
				...
				<>
					...
					{paymentType === "installment" && (
						<div
							...
							style={{ background: "#080818", border: "1px solid #1E2A40" }}
						>
							{/* Installment amount */}
							<div>
								<label className="block text-xs font-medium text-slate-400 mb-2">
								...
								<input
									...
									style={{
										background: "#1E1E38",
										border: errors.installmentAmount
											? "1px solid #EF4444"
											: "1px solid #1E2A40",
									}}
								/>
							...
							<div>
								...
								>
									{(
										["weekly", "monthly", "yearly"] as RecurringFrequency[]
									).map((freq) => (
										<button
											key={freq}
											...
											style={{
												background:
													installmentFrequency === freq
														...
														: "transparent",
												color:
													installmentFrequency === freq ? "#60A5FA" : "#64748B",
												border:
													installmentFrequency === freq
														? "1px solid #3B82F650"
