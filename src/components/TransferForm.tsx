...
export default function TransferForm({ onClose, editingTx }: Props) {
  const { language, accounts, transactions, savingsGoals, addTransfer, updateTransfer } = useStore();
  ...
  const toGoal = savingsGoals.find((g) => g.id === toId);
  const isSameCurrency = fromAccount?.currency === (toAccount?.currency ?? toGoal?.currency);
  ...
  const handleSave = () => {
    ...
    const toIsGoal = savingsGoals.some((g) => g.id === toId);

    if (toIsGoal) {
      // Delegate to contributeToSavingsGoal in store
      const { contributeToSavingsGoal } = useStore.getState();
      contributeToSavingsGoal(toId, fromId, parsedFromAmount, date);
      onClose();
      return;
    }
  ...
  return (
    <div className="px-5 pb-8 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col gap-2">
            ...
            {savingsGoals.filter((g) => g.status !== 'completed').map((goal) => (
              <button
                key={goal.id}
                onClick={() => setToId(goal.id)}
                className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm active-scale"
                style={{
                  background: toId === goal.id ? `${goal.color}22` : '#1E1E38',
                  border: toId === goal.id ? `1.5px solid ${goal.color}` : '1.5px solid #1E2A40',
                }}
              >
                <span>{goal.icon}</span>
                <div className="text-left flex-1 min-w-0">
                  <p className="text-slate-200 font-medium text-xs leading-tight truncate">{goal.name}</p>
                  <p className="text-slate-500 text-[10px]">
                    {formatAmount(goal.currentAmount, goal.currency)} / {formatAmount(goal.targetAmount, goal.currency)}
                  </p>
                </div>
                <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium" style={{ background: `${goal.color}20`, color: goal.color }}>
                  🎯
                </span>
              </button>
            ))}
      ...
      {fromAccount && (toAccount || toGoal) && fromId !== toId && (
        <div
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            {!isSameCurrency && (
              <div className="flex items-center gap-1 text-[10px] text-blue-400">
                <RefreshCw size={10} />
                <span>{fromAccount.currency} → {toAccount?.currency ?? toGoal?.currency}</span>
              </div>
            )}
          </div>
          <div className="text-center flex-1">
            <p className="text-xl">{toGoal ? toGoal.icon : toAccount?.icon}</p>
            <p className="text-xs text-slate-400 mt-0.5">{toGoal ? toGoal.name : toAccount?.name}</p>
          </div>
        </div>
      )}
