...
export default function ImportExportModal({ isOpen, onClose }: Props) {
	...
	async function handleExport(type: ExportType) {
		...
		try {
			...
			switch (type) {
				case "csv":
					savedPath = await exportToCSV(exportPayload);
					break;
				case "excel":
					savedPath = await exportToExcel(exportPayload);
					break;
				case "pdf":
					savedPath = await exportToPDF(exportPayload);
					break;
				case "json":
					savedPath = await exportToJSON(exportPayload);
					break;
			}
	...
	function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
		...
		const format: "csv" | "json" = file.name.toLowerCase().endsWith(".json")
			? "json"
			: "csv";
	...
	const canImport =
		...
		(importState.format === "json" ||
			importState.result.transactions.length > 0);

	return (
		<Modal
			isOpen={isOpen}
			onClose={onClose}
			title={isRu ? "Импорт / Экспорт" : "Import / Export"}
			fullHeight
		>
			<div className="px-5 pb-8 flex flex-col gap-4">
				...
				{(exportDone || importDone) && (
					...
					>
						...
						{tab === "export" && exportDone && Capacitor.isNativePlatform() && (
							<button
								onClick={() => FileSave.openExportsFolder().catch(() => {})}
								className="flex items-center justify-center gap-2 w-full py-2 rounded-xl text-xs font-semibold active-scale"
								style={{ background: "rgba(16,185,129,0.2)", color: "#34D399" }}
							>
								<FolderOpen size={14} />
								{isRu
									? "Открыть папку Megacalendar"
									: "Open Megacalendar folder"}
							</button>
						)}
					</div>
				)}
				...
				{tab === "export" && (
					<div className="space-y-3">
						...
						<ExportButton
							...
							subtitle={
								isRu
									? "Таблица с несколькими листами: транзакции, счета, бюджеты, долги"
									: "Multi-sheet: transactions, accounts, budgets, debts"
							}
						...
						<ExportButton
							icon={
								canExportNonJson ? <FileText size={20} /> : <Crown size={20} />
							}
							color={canExportNonJson ? "#EF4444" : "#F59E0B"}
							title={`PDF${canExportNonJson ? "" : " — Pro"}`}
							subtitle={
								isRu
									? "Красивый отчёт для печати или отправки"
						...
						<ExportButton
							icon={
								canExportNonJson ? <Download size={20} /> : <Crown size={20} />
							}
							color={canExportNonJson ? "#3B82F6" : "#F59E0B"}
							title={`CSV (.csv)${canExportNonJson ? "" : " — Pro"}`}
							subtitle={
								isRu
									? "Совместим с CoinKeeper, Money Manager и другими"
									: "Compatible with CoinKeeper, Money Manager, and more"
							}
				...
				{tab === "import" && !importState && (
					<div className="space-y-3">
						...
						>
							...
							<div className="text-center">
								<p className="text-sm font-semibold text-slate-200">
									{isRu ? "Выберите файл" : "Choose a file"}
								</p>
						...
						<div className="space-y-2">
							...
							{[
								{
									name: "FinCalendar CSV",
									desc: isRu ? "Наш экспорт" : "Our export",
								},
								{
									name: "CoinKeeper",
									desc: isRu ? "Экспорт операций" : "Transaction export",
								},
								{
									name: "Money Manager",
									desc: isRu ? "Отчёт CSV" : "CSV report",
								},
								{
									name: isRu ? "Любой CSV" : "Generic CSV",
									desc: "Date, Amount, Type, Description",
								},
							].map((app) => (
								<div
									key={app.name}
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-slate-500 mt-0.5">
									{formatFileSize(importState.fileSize)} ·{" "}
									{importState.format.toUpperCase()}
									{" · "}
