...
function CustomTooltip({ active, payload, currency, language }: CustomTooltipProps) {
  ...
  return (
    <div className="rounded-xl px-3 py-2 text-xs shadow-lg" style={{ background: '#1A1A2E', border: '1px solid #2A2A4A' }}>
      ...
      <p className="font-bold text-slate-100">{formatAmount(balance, currency)}</p>
      {change !== 0 && (
        <p style={{ color: change > 0 ? '#10B981' : '#EF4444' }}>
          {change > 0 ? '+' : ''}{formatAmount(change, currency)}
        </p>
      )}
    </div>
  );
...
export default function BalanceForecast() {
  const { accounts, plannedExpenses, debts, defaultCurrency, language } = useStore(useShallow((s) => ({
    accounts: s.accounts,
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium" style={{ color: delta >= 0 ? '#10B981' : '#EF4444' }}>
            ...
            {delta >= 0 ? '+' : ''}{formatAmount(delta, defaultCurrency)}
          </span>
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-lg font-bold text-slate-100">{formatAmount(currentBalance, defaultCurrency)}</p>
          </div>
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-bold" style={{ color: forecastBalance >= 0 ? '#F1F5F9' : '#EF4444' }}>
              {formatAmount(forecastBalance, defaultCurrency)}
            </p>
          </div>
          <div
        ...
        {willGoNegative && (
          <div className="rounded-xl px-3 py-2 mb-3 flex items-center gap-2" style={{ background: 'rgba(239,68,68,0.1)' }}>
            ...
            <p className="text-xs text-red-400">
              {isRu
                ? `Минимальный прогнозный баланс: ${formatAmount(minBalance, defaultCurrency)}`
                : `Minimum forecast balance: ${formatAmount(minBalance, defaultCurrency)}`}
            </p>
          </div>
        )}
