...
import {
	...
	usePricingModal,
} from "./contexts/PricingModalContext";
...
export default function App() {
		...
		const timer = setTimeout(() => {
			try {
				...
				for (let mo = -6; mo <= 6; mo++) {
					...
					for (const ds of Object.keys(getDebtPaymentsInMonth(debts, yr, mo2)))
						debtSet.add(ds);
					for (const ds of Object.keys(
						getScheduledPaymentsInMonth(debts, yr, mo2),
					))
						debtSet.add(ds);
					for (const ds of Object.keys(
						getDebtsWithDueDateInMonth(debts, yr, mo2),
					))
						debtSet.add(ds);
				}
				...
				/* non-Android or data error — skip widget update */
			}
		}, 1000);
	...
	const TABS = useMemo<
		{ mode: AddMode; label: string; icon: React.ReactNode; color: string }[]
	>(
		() => [
			{
				mode: "expense",
			...
			{
				mode: "transfer",
				label: language === "ru" ? "Перевод" : "Transfer",
	...
	const ENTITY_TABS = useMemo<
		{ type: EntityType; label: string; icon: React.ReactNode }[]
	>(
		() => [
			{
				type: "transaction",
				label: language === "ru" ? "Операция" : "Transaction",
				icon: <Receipt size={14} />,
			},
			{
				type: "debt",
				label: language === "ru" ? "Долг" : "Debt",
				icon: <Coins size={14} />,
			},
		],
		[language],
						...
						>
							{TABS.map((tab) => (
								<button
									...
									style={{
										background:
											addMode === tab.mode ? tab.color : "transparent",
			...
			<AddTransactionContext.Provider value={{ openAdd }}>
				...
				{isDesktop ? (
					...
					<div className="min-h-screen" style={{ background: "#07070F" }}>
						<Suspense
							fallback={
								<div style={{ height: "100vh", background: "#07070F" }} />
							}
						>
							{routes}
						</Suspense>
