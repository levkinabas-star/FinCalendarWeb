...
import { findPromoCode, calcProExpiry, type PromoPeriod } from "./promoCodes";
...
interface StoreState {
	...
	applyPromoCode: (
		...
		| { ok: true; period: PromoPeriod; labelRu: string; labelEn: string }
	...
	addPlannedExpense: (
		expense: Omit<PlannedExpense, "id" | "createdAt" | "completedDates">,
	) => void;
	...
	addDebt: (
		debt: Omit<
			...
			| "payments"
			| "scheduledPayments"
		> & { initialScheduledPayments?: Omit<ScheduledPayment, "id">[] },
	) => void;
	...
	addSavingsGoal: (
		goal: Omit<
			SavingsGoal,
			"id" | "createdAt" | "currentAmount" | "completedDates" | "status"
		>,
	) => void;
	updateSavingsGoal: (id: string, updates: Partial<SavingsGoal>) => void;
	deleteSavingsGoal: (id: string) => void;
	markSavingsGoalCompleted: (id: string, date: string) => void;
	updateSavingsGoalAmount: (id: string, delta: number) => void;
...
export const useStore = create<StoreState>()(
	persist(
		(set, get) => ({
			...
			setNotificationsLastChecked: (date) =>
				set({ notificationsLastChecked: date }),

			activatePro: () =>
				set({ plan: "pro", proExpiry: null, appliedPromoCode: null }),
			activateProWithExpiry: (expiry) =>
				set({ plan: "pro", proExpiry: expiry, appliedPromoCode: null }),
			...
			addTransaction: (tx) => {
				...
				if (account) {
					...
					get().updateAccount(tx.accountId, {
			...
			updateTransaction: (id, updates) => {
				...
				if (oldAccount) {
					...
					get().updateAccount(old.accountId, {
				...
				const newAccount = get().accounts.find(
					(a) => a.id === updates.accountId,
				);
				if (newAccount) {
					const delta =
						updates.type === "income" ? updates.amount : -updates.amount;
					get().updateAccount(updates.accountId, {
						balance: newAccount.balance + delta,
					});
				}
				set((s) => ({
			...
			deleteTransaction: (id) => {
				...
				if (tx) {
					...
					if (tx.transferId && tx.transferPeerId) {
						const peer = get().transactions.find(
							(t) => t.id === tx.transferPeerId,
						);
						if (peer) {
							const peerAccount = get().accounts.find(
								(a) => a.id === peer.accountId,
							);
							if (peerAccount) {
								// peer is the "incoming" side — reverse it
								get().updateAccount(peer.accountId, {
									balance: peerAccount.balance - peer.amount,
								});
							}
						}
						set((s) => ({
							transactions: s.transactions.filter(
								(t) => t.id !== id && t.id !== tx.transferPeerId,
							),
						}));
						return;
					}
			...
			updateTransfer: (
				outTxId,
				{ fromAccountId, toAccountId, fromAmount, toAmount, description, date },
			) => {
				const s = get();
				...
				set((state) => ({
					accounts: state.accounts.map((a) =>
						deltas[a.id] !== undefined
							? { ...a, balance: a.balance + deltas[a.id] }
							: a,
					),
			...
			togglePlannedCompleted: (id, date) => {
				...
				if (isCompleted) {
					...
					if (account) {
						...
						get().updateAccount(expense.accountId, {
							balance: account.balance + delta,
						});
					}
			...
			deleteCategory: (id) => {
				set((s) => ({
					categories: s.categories.filter((c) => c.id !== id || c.isPreset),
				}));
			},
			...
			addDebt: (debt) => {
				const { initialScheduledPayments, ...debtData } =
					debt as typeof debt & {
						initialScheduledPayments?: Omit<ScheduledPayment, "id">[];
					};
				...
				if (debtData.accountId) {
					const account = get().accounts.find(
						(a) => a.id === debtData.accountId,
					);
					if (account) {
						// lent = I gave money out → balance decreases
						// borrowed = I received money → balance increases
						const delta =
							debtData.direction === "lent"
								? -debtData.amount
								: debtData.amount;
						get().updateAccount(debtData.accountId, {
							balance: account.balance + delta,
						});
					}
				}
			...
			deleteDebt: (id) => {
				...
				if (debt?.accountId) {
					...
					if (account) {
						const delta =
							debt.direction === "lent" ? debt.amount : -debt.amount;
						get().updateAccount(debt.accountId, {
							balance: account.balance + delta,
						});
					}
				}
			...
			addDebtPayment: (debtId, payment) => {
				...
				const delta =
					debt.direction === "lent" ? payment.amount : -payment.amount;
			...
			updateDebtPayment: (debtId, paymentId, updates) => {
				...
				const oldAccount = get().accounts.find(
					(a) => a.id === oldPayment.accountId,
				);
				if (oldAccount) {
					const reverseDelta =
						debt.direction === "lent" ? -oldPayment.amount : oldPayment.amount;
					get().updateAccount(oldPayment.accountId, {
						balance: oldAccount.balance + reverseDelta,
					});
				}
				...
				if (newAccount) {
					const newDelta =
						debt.direction === "lent" ? updates.amount : -updates.amount;
					get().updateAccount(updates.accountId, {
			...
			deleteDebtPayment: (debtId, paymentId) => {
				...
				const delta =
					debt.direction === "lent" ? -payment.amount : payment.amount;
				get().updateAccount(payment.accountId, {
					balance: account.balance + delta,
				});
					...
					debts: s.debts.map((d) =>
						d.id === debtId
							? {
									...
									paidAmount: newPaidAmount,
									status: newStatus,
									scheduledPayments: newScheduled,
								}
							: d,
					),
				}));
				...
				set((s) => ({
					debts: s.debts.map((d) =>
						d.id === debtId
							? {
									...d,
									scheduledPayments: (d.scheduledPayments ?? []).filter(
										(p) => p.id !== scheduledId,
									),
								}
							: d,
			...
			markScheduledAsPaid: (debtId, scheduledId, accountId) => {
				const debt = get().debts.find((d) => d.id === debtId);
				if (!debt) return;
				const scheduled = (debt.scheduledPayments ?? []).find(
					(p) => p.id === scheduledId,
				);
				if (!scheduled) return;
				const account = get().accounts.find((a) => a.id === accountId);
				...
				const delta =
					debt.direction === "lent" ? scheduled.amount : -scheduled.amount;
				get().updateAccount(accountId, { balance: account.balance + delta });
							...
							? {
									...
									scheduledPayments: (d.scheduledPayments ?? []).map((sp) =>
										sp.id === scheduledId
											? {
													...sp,
													completedDates: (sp.completedDates ?? []).filter(
														(d) => d !== date,
													),
												}
											: sp,
			...
			markSavingsGoalCompleted: (id, date) => {
				set((s) => ({
					savingsGoals: s.savingsGoals.map((g) => {
						if (g.id !== id) return g;
						const isCompleted = g.completedDates.includes(date);
						return {
							...g,
							status: isCompleted ? "active" : "completed",
							completedDates: isCompleted
								? g.completedDates.filter((d) => d !== date)
								: [...g.completedDates, date],
						};
					}),
				}));
			},

			updateSavingsGoalAmount: (id, delta) => {
				set((s) => ({
					savingsGoals: s.savingsGoals.map((g) => {
						if (g.id !== id) return g;
						const newAmount = Math.max(0, g.currentAmount + delta);
						const isNowComplete = newAmount >= g.targetAmount;
						return {
							...g,
			...
			contributeToSavingsGoal: (goalId, fromAccountId, amount, date) => {
				const goal = get().savingsGoals.find((g) => g.id === goalId);
				const fromAccount = get().accounts.find((a) => a.id === fromAccountId);
				if (!goal || !fromAccount) return;
				const now = new Date().toISOString();
				...
				const outTx: Transaction = {
					...
					description:
						get().language === "ru"
							? `Накопление: ${goal.name}`
							: `Savings: ${goal.name}`,
					date,
					createdAt: now,
					transferId,
					transferPeerId: inId,
					transferPeerAccountId: goalId,
					transferRole: "out",
				...
				get().updateSavingsGoalAmount(goalId, amount);

				set((s) => ({ transactions: [outTx, inTx, ...s.transactions] }));
			},

			withdrawFromSavingsGoal: (goalId, toAccountId, amount, date) => {
				const goal = get().savingsGoals.find((g) => g.id === goalId);
				const toAccount = get().accounts.find((a) => a.id === toAccountId);
				if (!goal || !toAccount) return;
				const now = new Date().toISOString();
			...
			resetAllBalances: () => {
				set((s) => ({
					...
					transactions: [],
					// Reset planned expenses: unmark future scheduled but KEEP completed dates history
					plannedExpenses: s.plannedExpenses,
					// Keep budgets (they're limit structures, not operations)
					budgets: s.budgets,
					// Keep debts: preserve paidAmount, status, payments history and completed dates history
					debts: s.debts,
					// Keep savings goals: preserve completedDates history, reset currentAmount and status
					savingsGoals: s.savingsGoals.map((g) => ({
						...g,
						currentAmount: 0,
